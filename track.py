...
def intersect(A, B, C, D):
    print("inside intersect",A, B, C, D)
...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
                    ...
                    for box in boxes:
                        ...
                        if index_id[i] in previous:
                            ...
                            if intersect(p0, p1, line[0], line[1]):
                                print("Looks like there was an intersection")
                                # if p0's y coordinate is higher than p1's y coordinate
                                if p0[1] > p1[1]:
                                    people_counter_in += 1
                                else:
                                    people_counter_out +=1
                                print(people_counter_in,people_counter_out)
                                if label_list[i]=="PET clair bouteille":
                                  pet_count+=1
                                else:
                                  other_count+=1 
                        i += 1
            ...
            print("Count:",pet_count,other_count,pet_count+other_count)
            cv2.putText(im0, 'PET clair bouteille : {}'.format(pet_count),(40,50),cv2.FONT_HERSHEY_COMPLEX,1.0,(0,0,255),2)
            cv2.putText(im0, 'Others : {}'.format(other_count), (40,80),cv2.FONT_HERSHEY_COMPLEX,1.0,(0,0,255),2)
            cv2.putText(im0, 'Total Trash Detected : {}'.format(pet_count+other_count), (40,110),cv2.FONT_HERSHEY_COMPLEX,1.0,(0,0,255),2)
            ##########################################################################
